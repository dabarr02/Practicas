...
/*
Partes hechas:
	.- Añadir la opcion -c para poner las comas
	.-Añada la opción -i <inputfile> para especificar una ruta alternativa para el fichero passwd.
		Hacer una copia de /etc/passwd en otra ubicación para verificar el correcto funcionamiento de esta 
		nueva opción.
	.-Consulte la página de manual de la función strdup de la biblioteca estándar de C. 
		Intenta utilizar esta función como reemplazo de clone_string().
Partes que faltan:
...
int main(int argc, char *argv[])
{
	...
	while((opt = getopt(argc, argv, "hvpci:o:")) != -1) {
		switch(opt) {
		case 'h':
		...
		case 'c':
			...
			break;
		case 'i':
			if((options.inputfile=fopen(optarg,"r"))==NULL){
